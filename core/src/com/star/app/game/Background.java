...
public class Background {
    private class Star {
        ...
        public Star() {
            this.position = new Vector2(MathUtils.random(-200,
                    ScreenManager.SCREEN_WIDTH + 200),
        ...
        public void update(float dt) {
            position.x += (velocity.x -
                    gc.getHero().getVelocity().x / 10.0f) * dt;
            position.y += (velocity.y -
                    gc.getHero().getVelocity().y / 10.0f) * dt;
            if (position.x < -200) {
                position.x = ScreenManager.SCREEN_WIDTH + 20;
                position.y = MathUtils.random(-200,
    ...
    public void render(SpriteBatch batch) {
        ...
        for (int i = 0; i < stars.length; i++) {
            batch.draw(textureStar, stars[i].position.x - 8,
                    stars[i].position.y - 8, 8, 8, 16, 16, stars[i].scale,
                    stars[i].scale, 0, 0, 0, 16, 16, false, false);
            if (MathUtils.random(0, 500) < 2) {
                batch.draw(textureStar, stars[i].position.x - 8,
                        stars[i].position.y - 8, 8, 8, 16, 16,
                        stars[i].scale * 2, stars[i].scale * 2, 0, 0, 0, 16,
                        16, false, false);
            }
        }
    }
